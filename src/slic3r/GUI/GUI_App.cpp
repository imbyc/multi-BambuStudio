...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    if (data_dir().empty()) {
        // BBS: Get executable directory name for data directory customization
        std::string exe_dir_name = "";
        #ifdef __WINDOWS__
            wchar_t exe_path[MAX_PATH];
            if (GetModuleFileNameW(NULL, exe_path, MAX_PATH) != 0) {
                boost::filesystem::path exe_full_path(exe_path);
                exe_dir_name = exe_full_path.parent_path().filename().string();
            }
        #endif
        
        #ifndef __linux__
            std::string base_user_data_dir = wxStandardPaths::Get().GetUserDataDir().ToUTF8().data();
            // Extract base directory path (remove "BambuStudio" part)
            boost::filesystem::path base_path(base_user_data_dir);
            std::string parent_dir = base_path.parent_path().string();
            
            // Create custom data directory name based on executable directory
            std::string custom_app_name = GetAppName().ToStdString();
            if (!exe_dir_name.empty() && exe_dir_name != "Bambu Studio") {
                // Replace spaces with no space and keep the custom name
                custom_app_name = exe_dir_name;
                std::replace(custom_app_name.begin(), custom_app_name.end(), ' ', '_');
            }
            m_base_data_dir = (boost::filesystem::path(parent_dir) / custom_app_name).string();
        #else
            ...
            m_base_data_dir = (boost::filesystem::path(dir.ToStdString()) / custom_app_name).string();
        #endif
#if BBL_INTERNAL_TESTING
            m_base_data_dir += BBL_INTERNAL_TESTING == 1 ? "Internal" : "Beta";
#endif
            // BBS: Set initial data directory to base directory
            //BBS create folder if not exists
            boost::filesystem::path data_dir_path(m_base_data_dir);
            if (!boost::filesystem::exists(data_dir_path))
                boost::filesystem::create_directory(data_dir_path);
            set_data_dir(m_base_data_dir);
#if defined(__WINDOWS__)
            // Change current dirtory of application
            _chdir(encode_path((m_base_data_dir + "/log").c_str()).c_str());
#else
            chdir(encode_path((m_base_data_dir + "/log").c_str()).c_str());
#endif
    } else {
...
// BBS: User-specific data directory management
void GUI_App::switch_to_user_data_dir(const std::string& user_id)
{
    if (user_id.empty()) {
        BOOST_LOG_TRIVIAL(warning) << "switch_to_user_data_dir: user_id is empty";
        return;
    }
    
    if (m_current_user_id == user_id) {
        BOOST_LOG_TRIVIAL(info) << "Already using data directory for user: " << user_id;
        return;
    }
    
    // Create user-specific data directory path
    std::string user_data_dir = m_base_data_dir + "/user/" + user_id;
    boost::filesystem::path user_data_dir_path(user_data_dir);
    
    try {
        if (!boost::filesystem::exists(user_data_dir_path)) {
            boost::filesystem::create_directories(user_data_dir_path);
            BOOST_LOG_TRIVIAL(info) << "Created user data directory: " << user_data_dir;
        }
        
        // Create subdirectories if they don't exist
        std::vector<std::string> subdirs = {"log", "printers", "presets", "cache"};
        for (const auto& subdir : subdirs) {
            boost::filesystem::path subdir_path = user_data_dir_path / subdir;
            if (!boost::filesystem::exists(subdir_path)) {
                boost::filesystem::create_directory(subdir_path);
            }
        }
        
        // Switch to user-specific data directory
        set_data_dir(user_data_dir);
        m_current_user_id = user_id;
        
        // Update log directory
#if defined(__WINDOWS__)
        _chdir(encode_path((user_data_dir + "/log").c_str()).c_str());
#else
        chdir(encode_path((user_data_dir + "/log").c_str()).c_str());
#endif
        
        // Reload app config from user-specific location
        if (app_config) {
            app_config->save(); // Save current config before switching
            delete app_config;
            app_config = nullptr;
        }
        app_config = new AppConfig();
        if (app_config->exists()) {
            std::string error = app_config->load();
            if (!error.empty()) {
                BOOST_LOG_TRIVIAL(error) << "Failed to load user-specific config: " << error;
            }
        } else {
            // Set default values for new user config
            app_config->set_defaults();
        }
        
        BOOST_LOG_TRIVIAL(info) << "Switched to user data directory: " << user_data_dir;
        
    } catch (const std::exception& e) {
        BOOST_LOG_TRIVIAL(error) << "Failed to switch to user data directory: " << e.what();
    }
}

void GUI_App::restore_default_data_dir()
{
    if (m_current_user_id.empty()) {
        BOOST_LOG_TRIVIAL(info) << "Already using default data directory";
        return;
    }
    
    // Switch back to base data directory
    set_data_dir(m_base_data_dir);
    m_current_user_id.clear();
    ...
    if (app_config->exists()) {
        std::string error = app_config->load();
        if (!error.empty()) {
            BOOST_LOG_TRIVIAL(error) << "Failed to load default config: " << error;
        }
    }
    
    BOOST_LOG_TRIVIAL(info) << "Restored to default data directory: " << m_base_data_dir;
}

std::string GUI_App::get_current_user_id() const
{
    if (m_agent && m_agent->is_user_login()) {
        return m_agent->get_user_id();
    }
    return "";
}
...
void GUI_App::request_user_logout()
{
    if (m_agent && m_agent->is_user_login()) {
        ...
        // BBS: Data directory remains isolated by executable directory
        BOOST_LOG_TRIVIAL(info) << "User logged out, data directory remains: " << data_dir();
...
void GUI_App::on_user_login_handle(wxCommandEvent &evt)
{
    ...
    // BBS: Data directory is already isolated by executable directory name
    BOOST_LOG_TRIVIAL(info) << "Using data directory: " << data_dir();
