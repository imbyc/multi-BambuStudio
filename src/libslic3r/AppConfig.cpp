...
{
#ifdef USE_JSON_CONFIG
    std::string filename = (m_mode == EAppMode::Editor) ? 
        (SLIC3R_APP_KEY ".conf") : (GCODEVIEWER_APP_KEY ".conf");
#else
    std::string filename = (m_mode == EAppMode::Editor) ? 
        (SLIC3R_APP_KEY ".ini") : (GCODEVIEWER_APP_KEY ".ini");
#endif

    std::string path = (boost::filesystem::path(Slic3r::data_dir()) / filename).make_preferred().string();
    return path;
}
